---
Layout: single-page
Title: Serviços
---

<section class="sec-videos">
    <div class="cxp-videos">
        <a href="{{'/playlist-1/' | relative_url}}" class="ct-pasta-videos">
            <div class="pasta-videos">
                <img src="{{'/assets/img/icone-close-folder3.png' | relative_url}}" class="img-original">
                <img src="{{'/assets/img/icone-open-folder3.png' | relative_url}}" class="img-hover">
            </div>
            <div class="vid-desc">
                <h3>Introdução à Grande Estética de Lukács</h3>
                <p>Ministrante: Professor Dr. Deribaldo Santos - Coletivo Veredas (2022)</p>
            </div>
        </a>
        <a href="{{'/circulos-formativos/' | relative_url}}" class="ct-pasta-videos">
            <div class="pasta-videos">
                <img src="{{'/assets/img/icone-close-folder3.png' | relative_url}}" class="img-original">
                <img src="{{'/assets/img/icone-open-folder3.png' | relative_url}}" class="img-hover">
            </div>
            <div class="vid-desc">
                <h3>Círculos Formativos</h3>
                <p>Ministrante: Vários - EMANCIPA/IMO (2022)</p>
            </div>
        </a>
        <a href="{{'/circulo-dominical/' | relative_url}}" class="ct-pasta-videos">
            <div class="pasta-videos">
                <img src="{{'/assets/img/icone-close-folder3.png' | relative_url}}" class="img-original">
                <img src="{{'/assets/img/icone-open-folder3.png' | relative_url}}" class="img-hover">
            </div>
            <div class="vid-desc">
                <h3>Circulo Dominical</h3>
                <p>Ministrante: Sérgio Lessa - Coletivo Veredas (2017)</p>
            </div>
        </a>
        <a href="{{'/circulo-dominical-quarentena/' | relative_url}}" class="ct-pasta-videos">
            <div class="pasta-videos">
                <img src="{{'/assets/img/icone-close-folder3.png' | relative_url}}" class="img-original">
                <img src="{{'/assets/img/icone-open-folder3.png' | relative_url}}" class="img-hover">
            </div>
            <div class="vid-desc">
                <h3>Circulo Dominical de Quarentena</h3>
                <p>Ministrante: Sérgio Lessa - Coletivo Veredas (2020)</p>
            </div>
        </a>
        <a href="{{'/#/' | relative_url}}" class="ct-pasta-videos">
            <div class="pasta-videos">
                <img src="{{'/assets/img/icone-close-folder3.png' | relative_url}}" class="img-original">
                <img src="{{'/assets/img/icone-open-folder3.png' | relative_url}}" class="img-hover">
            </div>
            <div class="vid-desc">
                <h3>Aguardando conteúdo...</h3>
                <p>Ministrante: xxxx - xxxx (xxxx)</p>
            </div>
        </a>
        <a href="{{'/#/' | relative_url}}" class="ct-pasta-videos">
            <div class="pasta-videos">
                <img src="{{'/assets/img/icone-close-folder3.png' | relative_url}}" class="img-original">
                <img src="{{'/assets/img/icone-open-folder3.png' | relative_url}}" class="img-hover">
            </div>
            <div class="vid-desc">
                <h3>Aguardando conteúdo...</h3>
                <p>Ministrante: xxxx - xxxx (xxxx)</p>
            </div>
        </a>
    </div>
</section>